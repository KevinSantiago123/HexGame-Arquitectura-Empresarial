...
class Game:
    ...
    def loadData(self):
        ...
        #-----------------TEXT Info--------------------
        with open(path.join(doc_folder, INFO), 'r') as f:
            self.info_text = ''.join(f.readlines())
    ...
    def tick(self, pos):
        ...
        self.validar = self.move
        self.move = self.ques.preguntar(self.move)
    ...
    def startScreen(self):
        ...
        play = Button((W/2, 2.1*H/3), 70, 'Jugar')
        settings = Button((150, H-75), 40, 'Configuración')
        rules = Button((W-100, H-75), 40, 'Reglas')
        ...
        while start:
            ...
            textOut(self.screen, 'HEX', 100, ORANGE, (W/2, H/3))
            textOutMultiline(self.screen, 'Gerencia Estrategica', 25, GREEN, (W/2, H/3-30))
            textOutMultiline(self.screen, self.info_text, 18, ORANGE, (W/2, H/3+30))
            # show buttons
            for button in buttons:
    ...
    def rulesScreen(self):
        ...
        while start:
            ...
            # --------------------STUFF-----------------------
            self.screen.blit(self.bg_img, (0, 0))
            textOut(self.screen, 'Reglas', 100, ORANGE, (W/2, H/3))
            textOutMultiline(self.screen, self.rules_text, 20, BLACK, (W/2, H/3))
            # show buttons
            for button in buttons:
                button.show(self.screen)
            # double processing
    ...
    def settingsScreen(self):
        ...
        while start:
            ...
            # --------------------STUFF-----------------------
            self.screen.blit(self.bg_img, (0, 0))
            textOut(self.screen, 'Configuración', 100, ORANGE, (W/2, H/4))
            textOut(self.screen, 'Tamaño del tablero:', 40, BLACK, (W/3, H/2))
            textOut(self.screen, self.size, 40, BLACK, (2*W/3, H/2))
            textOut(self.screen, 'Música:', 40, BLACK, (W/3, H/2+60))
            textOut(self.screen, 'Sonido:', 40, BLACK, (W/3, H/2+120))
    ...
    def pauseScreen(self):
        ...
        start = True
        # initializing buttons
        resume = Button((W/2, H/3), 80, 'Resumen', col=ORANGE)
        home = Button((W/2, H/2), 50, 'Inicio', col=WHITE)
        buttons = [home, resume]
        while start:
            # sticking to fps
            self.clock.tick(FPS)
    ...
    def GOScreen(self, winner):
        ...
        go = True
        home = Button((W/2, 2*H/3), 30, 'Inicio', col=WHITE)
        while go:
            # sticking to fps
            self.clock.tick(FPS)
            # --------------------EVENTS---------------------
            ...
            self.showGrid()
            self.shadow()
            textOut(self.screen, 'Juego Terminado', 80, ORANGE, (W/2, H/3))
            if winner == 2:
                textOut(self.screen, 'Ganador -> Azul', 50, BLUE, (W/2, H/2))
            else:
                textOut(self.screen, 'Ganador -> Verde', 50, GREEN, (W/2, H/2))
